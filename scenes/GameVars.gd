extends Node

const SCREEN_WIDTH = 1024
const SCREEN_HEIGHT = 600

const GROUP_GROUNDS = "grounds"
const GROUP_OBSTACLES = "obstacles"
const GROUP_COINS = "coins"

const STAGE_SPLASH = "res://scenes/splash/Splash.tscn"
const STAGE_GAMEPLAY = "res://scenes/gameplay/GamePlay.tscn"

const SAVE_GAME_NAME_GENERAL = "MyGameBerhitung_general"
const SAVE_GAME_NAME_SCOREBOARD = "MyGameBerhitung_scoreboard"

const PREFABS_EXIT_CONFIRMATION = "res://scenes/exitconfirmation/ExitConfirmation.tscn"
const PREFABS_INFORMASI = "res://scenes/informasi/Informasi.tscn"
const PREFABS_ABOUT = "res://scenes/about/About.tscn"
const PREFABS_SETTING = "res://scenes/setting/Setting.tscn"

const DIFFICULTY_EASY = 0
const DIFFICULTY_MEDIUM = 1
const DIFFICULTY_HARD = 2